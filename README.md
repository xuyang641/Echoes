...
- **梦境画师 (Dream Painter)**：根据你的日记文字，自动生成独一无二的艺术配图。
- **心情歌单 (Mood Playlist)**：分析你当下的心境，为你推荐最懂你的音乐。
- **每日回顾 (Daily Summary)**：智能总结一天的经历，帮你发现生活中的美好模式。

### ☁️ 离线优先 & 云端同步
Echoes 采用先进的 **Offline-First** 架构。无论在深山还是飞行模式，你都可以毫无阻碍地浏览和写作。网络恢复后，所有更改会自动在后台静默同步到 Supabase 云端。
...
3. **配置环境变量**
   ...
   # Set to true if using Supabase Edge Functions for security
