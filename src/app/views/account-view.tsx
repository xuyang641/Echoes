...
import { User, Mail, UserPlus, Check, X, Trash2, LogOut, Settings, Globe, Bell, MapPin, Camera, BookOpen, Calendar, Coffee, ExternalLink, Copy, Share2, Users } from 'lucide-react';
...
import { BackupManager } from '../components/managers/backup-manager';
import { SecurityManager } from '../components/managers/security-manager';
import { StorageManager } from '../components/managers/storage-manager';
import { ThemeManager } from '../components/managers/theme-manager';
...
export function AccountView({ entries }: AccountViewProps) {
  const { user, signOut } = useAuth();
  const { friends, friendRequests, sendFriendRequest, acceptFriendRequest, rejectFriendRequest, removeFriend } = useFriend();
  const { groupRequests, acceptGroupRequest, rejectGroupRequest } = useGroup();
    ...
    >
      ...
      {activeTab === 'profile' ? (
        <motion.div variants={containerVariants} className="space-y-6">
          ...
          <motion.div variants={itemVariants}>
            <NotificationManager>
                {({ settings, updateSettings }: { settings: any, updateSettings: any }) => (
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                    ...
                    {settings.enabled && (
                        <div className="flex items-center gap-4 mt-4 w-full">
                          <input 
                            ...
                            className="px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white shrink-0"
                          />
                          <input 
                            type="text" 
                            placeholder={t('notifications.reminderBody', '今天过得怎么样？在记忆褪色前记录下这一刻吧。')}
                            value={settings.customText || ''}
                            onChange={(e) => updateSettings({ ...settings, customText: e.target.value })}
                            className="flex-1 px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white"
                          />
                        </div>
                    )}
                    </div>
                    ...
                    {settings.enabled && (
                      <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                        <span className="text-sm text-gray-600 dark:text-gray-300">
                          {t('notifications.autostart', '开机自启动 (确保后台通知)')}
                        </span>
                        <div className="relative inline-block w-10 h-5 transition duration-200 ease-in-out rounded-full cursor-pointer">
                          <input 
                              ...
                              onChange={(e) => updateSettings({ ...settings, autostartEnabled: e.target.checked })}
                          />
                          <label 
                              htmlFor="toggle-autostart"
                              className="block w-10 h-5 overflow-hidden bg-gray-200 rounded-full cursor-pointer peer-checked:bg-blue-500 transition-colors"
                          ></label>
                          <span className="absolute left-0.5 top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform peer-checked:translate-x-5"></span>
                        </div>
                      </div>
                    )}
        ...
        <motion.div variants={containerVariants} className="space-y-6">
          ...
          {groupRequests.length > 0 && (
            <motion.div variants={itemVariants} className="bg-purple-50 dark:bg-purple-900/20 rounded-2xl p-6 border border-purple-100 dark:border-purple-800">
              ...
              <div className="space-y-3">
                {groupRequests.map(request => (
                  <div key={request.id} className="flex items-center justify-between bg-white dark:bg-gray-800 p-3 rounded-xl shadow-sm">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium text-gray-900 dark:text-white">
                          {request.from.name} 邀请你加入 <span style={{color: request.group.color}}>{request.group.name}</span>
                        </div>
                        <div className="text-xs text-gray-500">{request.from.email}</div>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button 
                        onClick={() => acceptGroupRequest(request.id)}
                        className="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200 transition-colors"
                        title="接受"
                      >
                        <Check className="w-4 h-4" />
                      ...
                      <button 
                        onClick={() => rejectGroupRequest(request.id)}
                        className="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors"
                        title="拒绝"
                      >
                        <X className="w-4 h-4" />
