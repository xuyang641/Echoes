...
3. **需要上架国内安卓应用商店**：各大应用商店（华为、小米）强制要求你的服务器必须有 **ICP 备案**。由于 Vercel 是海外服务器，无法备案。此时你必须购买国内的服务器来走备案流程。
4. **数据绝对隐私**：你希望用户的所有日记和照片只存在你自己的服务器上，不放在 MemFire 这样的第三方平台上。

**平滑升级**：因为代码完全没变，未来升级时，你只需要将 `.env` 里的 `VITE_SUPABASE_URL` 换成你自己在国内云服务器上部署的 Supabase 地址即可。